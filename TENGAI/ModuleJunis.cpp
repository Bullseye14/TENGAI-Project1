...
bool ModuleJunis::CleanUp()
{
	...
	/*App->audio->UnloadSFX(JunisShot);
	App->audio->UnloadSFX(JunisCollision);
	App->audio->UnloadSFX(JunisPowerDown);
	App->audio->UnloadSFX(JunisPowerUp);
	App->audio->UnloadSFX(SocratesAttack);	   */
...
bool ModuleJunis::Spawn() {
	...
	if (!Spawn_Animation)
	{
		...
		position = iPoint(-App->render->camera.x -25, 150);
		player_collider = App->collision->AddCollider({ position.x, position.y, 27, 28 }, COLLIDER_PLAYER, this);
